...
namespace GestionStages.Controllers
{
    ...
    public class EtudiantsController : Controller
    {
        ...
        // GET: Etudiants - AVEC RECHERCHE, FILTRAGE ET PAGINATION (10 par page)
        [Authorize(Roles = "Admin,Etudiant")]
        public async Task<IActionResult> Index(string searchString, string filiereFilter, string niveauFilter, int pageNumber = 1)
        {
            ...
            // Liste des filières disponibles pour le filtre (dropdown)
            ...
            // Liste des niveaux disponibles pour le filtre (dropdown)
            ...
            if (User.IsInRole("Etudiant"))
            {
                ...
                // Pour l'étudiant : pas de pagination, il n'a qu'un seul profil
                var etudiantProfil = await etudiantsQuery.SingleOrDefaultAsync();
                return View(new List<Etudiant> { etudiantProfil ?? new Etudiant() });
            }
            ...
            if (!string.IsNullOrEmpty(searchString))
            {
                ...
                etudiantsQuery = etudiantsQuery.Where(e =>
                    ...
                    e.Prenom.ToLower().Contains(searchString) ||
            ...
            // Nombre total (Admin)
            var totalEtudiants = await etudiantsQuery.CountAsync();

            // Pagination (Admin)
            var etudiants = await etudiantsQuery
            ...
            ViewBag.TotalEtudiants = totalEtudiants;
            ViewBag.HasPreviousPage = pageNumber > 1;
            ViewBag.HasNextPage = pageNumber < Math.Ceiling((double)totalEtudiants / pageSize);
